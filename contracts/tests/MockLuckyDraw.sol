...
contract MockLuckyDraw is Ownable {
  ...
  uint256 public randomnessForGrandPrize;
  uint256 public randomnessForFirstPrize;
  uint256 public randomnessForSecondPrize;
  uint256 public randomnessForThirdPrize;
  uint256 public randomnessForFourthPrize;
  ...
  event RandomnessUpdated(uint256 prizeType, uint256 blockNumber, uint256 randomness);
  ...
  constructor() {
    ...
    //test
    randomnessForGrandPrize = 660517705;
    randomnessForFirstPrize = 660517705;
    randomnessForSecondPrize = 4198062867;
    randomnessForThirdPrize = 3704674354;
    randomnessForFourthPrize = 4144872107;
  }
  ...
  function fetchRandomNumber(uint256 prizeType) external {
    ...
    uint256 randomValue = witnet.random(type(uint32).max, 0, blockNumber);
    if (prizeType == 1) {
   ...
   */
  function getRandomNumber(uint256 prizeType) external view returns (uint32) {
    uint256 blockNumber;
    if (prizeType == 1) {
      blockNumber = blockForFirstPrize;
    } else if (prizeType == 2) {
    ...
    require(blockNumber > 0, "Lottery: pending randomize");
    return uint32(witnet.random(type(uint32).max, 0, blockNumber));
  }
  ...
  function random(
    ...
    return uint256(keccak256(abi.encodePacked(prizeType, randomValue, i, block.number, block.timestamp)));
  }

  function pickWinnerForGrand(uint256 prizeType) internal {
    uint256 loopIndex;
    uint256 randomValue = randomnessForGrandPrize;
    require(randomValue > 0, "Lottery: pending randomize");
    for (uint256 i = 0; i < TOTAL_PARTICIPATION; i++) {
      uint256 randomNumber = 1 + (random(prizeType, randomValue, i) % TOTAL_PARTICIPATION);
  ...
  function pickWinnerForFirst(uint256 prizeType) internal {
    uint256 loopIndex;
    uint256 randomValue = randomnessForFirstPrize;
    require(randomValue > 0, "Lottery: pending randomize");
    for (uint256 i = 0; i < TOTAL_PARTICIPATION; i++) {
      uint256 randomNumber = 1 + (random(prizeType, randomValue, i) % TOTAL_PARTICIPATION);
  ...
  function pickWinnerForSecond(uint256 prizeType) internal {
    uint256 loopIndex;
    uint256 randomValue = randomnessForSecondPrize;
    require(randomValue > 0, "Lottery: pending randomize");
    for (uint256 i = 0; i < TOTAL_PARTICIPATION; i++) {
      uint256 randomNumber = 1 + (random(prizeType, randomValue, i) % TOTAL_PARTICIPATION);
  ...
  function pickWinnerForThird(uint256 prizeType) internal {
    uint256 loopIndex;
    uint256 randomValue = randomnessForThirdPrize;
    require(randomValue > 0, "Lottery: pending randomize");
    if (lists.length > 0) {
      delete (lists);
    }
  ...
  function pickWinnerForFourth(uint256 prizeType) internal {
    uint256 loopIndex;
    uint256 randomValue = randomnessForFourthPrize;
    require(randomValue > 0, "Lottery: pending randomize");
    if (lists.length > 0) {
      delete (lists);
    }
