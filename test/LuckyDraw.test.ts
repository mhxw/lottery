...
describe('LuckyDraw', () => {
  ...
  describe('#pickWinner', () => {
    ...
    it('enter a non-existent prize by call pickWinner', async () => {
      await evmRevert(
          lottery.connect(personas.Carol).pickWinner(5),
          'Lottery: the prize does not exist',
      )
    })
  })
  ...
  describe('#requestnumber', () => {
    ...
    it('enter a non-existent prize by call requestNumber', async () => {
      await evmRevert(
          lottery.connect(personas.Carol).requestRandomNumber(5,{ value: ethers.utils.parseEther('0.5') }),
          'Lottery: the prize does not exist',
      )
    })
    ...
    it('request random number success', async () => {
      ...
      assert.equal(await lottery.randomnessForFourthPrize(), randomValue)
